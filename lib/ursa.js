...
function createPublicKeyFromParameters(parameters) {
    if (!parameters || !parameters.modulus) {
        throw new Error("Wrong parameters");
    }
    parameters.exponent = parameters.exponent || new Buffer('AQAB', 'base64');
    var rsa = new RsaWrap();
    rsa.setPublicParameters(parameters.modulus, parameters.exponent);
    return PublicKey(rsa);
}
...
function createPrivateKeyFromParameters(parameters) {
    if (!parameters || !parameters.modulus || !parameters.d ||
            !parameters.p || !parameters.q || !parameters.dmp ||
            !parameters.dmq || !parameters.iqmp) {
        throw new Error("Wrong parameters");
    }
    parameters.exponent = parameters.exponent || new Buffer('AQAB', 'base64');
    var rsa = new RsaWrap();
    rsa.setPrivateParameters(
        parameters.modulus, parameters.exponent,
        parameters.d, parameters.p, parameters.q, parameters.dmp,
        parameters.dmq, parameters.iqmp
    );
    return PrivateKey(rsa);
}
...
module.exports = {
    ...
    createPrivateKeyFromParameters: createPrivateKeyFromParameters,
    createPublicKey:        createPublicKey,
    createPublicKeyFromParameters: createPublicKeyFromParameters,
